...
// Bulk-create brand-new roster rows with full_name/institution_id/cohort_id
// set directly — used by the institution admin's CSV/roster import (distinct
// from /bulk-upsert above, which is the lecturer enroll-by-number flow and
// only ever creates bare id+student_number rows). The caller has already
// filtered out student_numbers that already exist anywhere, so this only
// ever inserts genuinely new rows — no upsert/merge semantics needed here.
router.post('/bulk-insert', requireAuth, async (req, res, next) => {
  try {
    const { students: rows } = req.body || {};
    if (!Array.isArray(rows) || !rows.length) return res.status(400).json({ error: 'students[] is required' });

    const institutionId = rows[0]?.institution_id;
    if (!institutionId || rows.some((r) => r.institution_id !== institutionId)) {
      return res.status(400).json({ error: 'All rows must share the same institution_id' });
    }
    if (!isPlatformOrInstitutionAdmin(req.user, institutionId)) {
      return res.status(403).json({ error: 'Forbidden' });
    }
    if (rows.some((r) => !r.student_number)) {
      return res.status(400).json({ error: 'Every row requires a student_number' });
    }
    ...
    await db('students').insert(toInsert);
    res.status(201).json(await db('students').whereIn('id', toInsert.map((r) => r.id)));
  } catch (err) {
    next(err);
  }
});
