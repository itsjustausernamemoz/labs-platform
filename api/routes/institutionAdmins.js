...
const { requireAuth, hashPassword, signResetToken } = require('../lib/auth');
...
// Institution admins always land back on this app to accept an invite or
// reset their password — unlike lecturer/student flows there's no
// per-deployment app URL to infer, so it's fixed here rather than passed
// through from the caller.
const INSTITUTION_ADMIN_APP_URL = 'https://mashoke-institution-admin.web.app';
...
//
// No password is accepted from the caller — a random one is generated and
// immediately discarded (never returned, never logged) so nobody but the
// invitee ever knows it. The invitee sets their own password by following
// the emailed link, same mechanism as "forgot password" but with a longer
// (24h) expiry since accepting an invite isn't as time-sensitive.
router.post('/', requireAuth, async (req, res, next) => {
  try {
    const body = req.body || {};
    const { institution_id, email, full_name } = body;
    if (!institution_id || !email || !full_name) {
      return res.status(400).json({ error: 'institution_id, email and full_name are required' });
    }
    ...
    const resetToken = signResetToken(id, '24h');
    const link = `${INSTITUTION_ADMIN_APP_URL}/reset-password?token=${resetToken}`;
    sendMail({
      to: email,
      subject: "You've been invited to Mashoke Labs",
      text: `Hi ${full_name},\n\nYou've been invited as an institution admin on Mashoke Labs. Set your password to get started:\n\n${link}\n\nThis link expires in 24 hours. If you weren't expecting this, you can ignore this email.`,
    }).catch((err) => console.error('[mail] invite email failed:', err.message));
